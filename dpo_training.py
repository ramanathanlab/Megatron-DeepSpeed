...
def batch_seq_logprobs(logits, labels):
    ...
    logprobs = torch.gather(
        logits_logsoftmax, dim=2, index=labels.unsqueeze(2)
    ).squeeze(2)
...
def calculate_dpo_loss(
    ...
    if teacher_model:
        ...
        # Labels ?
        logprobs = torch.gather(
            student_logits, dim=2, index=labels.unsqueeze(2)
        ).squeeze(2)
...
def compute_dp_loss(logprobs_p, ref_logprobs_p, logprobs_u, ref_logprobs_u, beta=0.1):
    # Get ratios of preferred log probabilities from model and ref model
    logprob_ratio_p = logprobs_p - ref_logprobs_p
...
def main():
    ...
    if os.getenv("TORCH_PROFILER_ENABLED") == "1":
        ...
        prof.export_chrome_trace(
            f"{args.tensorboard_dir}/torch-trace-{RANK}-of-{WORLD_SIZE}.json"
        )
    else:
        # Initalize and get arguments, timers, and Tensorboard writer.
        initialize_megatron(
        ...
        if isinstance(model_ref, deepspeed.PipelineEngine):
            ...
            assert (
                model_ref.grid.get_pipe_parallel_rank()
                == mpu.get_pipeline_model_parallel_rank()
            )
            assert (
                model_ref.grid.get_slice_parallel_rank()
                == mpu.get_tensor_model_parallel_rank()
            )
            assert (
                model_ref.grid.get_data_parallel_rank() == mpu.get_data_parallel_rank()
            )

        model_ref = [model_ref]
        iteration2 = load_checkpoint(
            model_ref, optimizer_2, opt_param_scheduler_2
        )  # THIS WORKED!! After commenting out assert args.consumed_train_samples == 0 in load_checkpoint()
        ...
        timers = get_timers()
        assert timers is not None
        assert isinstance(gas, int) and gas >= 1
        for epoch in range(1):
            iteration = 0
            for train_iter in range(args.train_iters):
                # Get batch
                _t0 = time.perf_counter()  # tick: data start
                for micro_step in range(gas):
                    ...
                    loss_c = tensor_parallel.vocab_parallel_cross_entropy(
                        output_c.contiguous().float(), labels_c
                    )
                    # Reference model forward with concatenated inputs
                    with torch.no_grad():
                        # Logits and loss
                        routput_c, _ = model_ref[0](
                            tokens_c,
                            position_ids_c,
                            None,
                            # labels=labels_u
                        ...
                        rloss_c = tensor_parallel.vocab_parallel_cross_entropy(
                            routput_c.contiguous().float(), labels_c
                        )
                    _tf1 = time.perf_counter()  # tock: forward stop
                    # Seq logprobs
                    seq_logps_p = torch.sum(
                        loss_c[: args.micro_batch_size, :] * loss_mask_p, dim=-1
                    ) / torch.sum(loss_mask_p, dim=-1)
                    seq_logps_u = torch.sum(
                        loss_c[args.micro_batch_size :, :] * loss_mask_u, dim=-1
                    ) / torch.sum(loss_mask_u, dim=-1)
                    rseq_logps_p = torch.sum(
                        rloss_c[: args.micro_batch_size, :] * loss_mask_p, dim=-1
                    ) / torch.sum(loss_mask_p, dim=-1)
                    rseq_logps_u = torch.sum(
                        rloss_c[args.micro_batch_size :, :] * loss_mask_u, dim=-1
                    ) / torch.sum(loss_mask_u, dim=-1)
                ...
                if (
                    (train_iter % args.save_interval == 0)
                    and (train_iter > 0)
        ...
        if False:
            ...
            if mpu.is_pipeline_last_stage():
                if return_output_log_probs:
                    ...
                    print_rank_0(
                        f"On mpu.is_pipeline_last_stage branch and output_log_probs is set: {output_log_probs}"
                    )
                ...
                print_rank_0(
                    f"On mpu.is_pipeline_last_stage branch and generated_sequence_lengths: {generated_sequence_lengths}"
                )
            ...
            with torch.no_grad():
                prompts_attention_mask, _, prompts_position_ids = (
                ...
                for context_length in range(min_prompt_length, max_sequence_length):
                    ...
                    tokens2use = prompts_tokens_tensor[
                        :, prev_context_length:context_length
                    ]
                    positions2use = prompts_position_ids[
                        :, prev_context_length:context_length
                    ]
                    ...
                    # -------
                    inference_params = InferenceParams(batch_size, tokens2use.size(1))
                    plogits = model[0](
                        tokens2use,
                        positions2use,
                        attention_mask2use,
                    ...
                    if mpu.is_pipeline_last_stage():
                        ...
                        for nidx, ns in enumerate(new_sample.cpu().numpy().tolist()):
                            print_rank_0(
                                f"nidx: {nidx}, new_sample[{nidx}]: {tokenizer.detokenize(ns)}"
                            )
                        ...
                        for nidx, ns in enumerate(
                            prompts_tokens_tensor.cpu().numpy().tolist()
                        ):
                            print_rank_0(
                                f"nidx: {nidx}, prompts_tokens_tensor before[{nidx}]: {tokenizer.detokenize(ns)}"
                            )
                        prompts_tokens_tensor[started, context_length] = new_sample[
                            started
                        ]
                        ...
                        for nidx, ns in enumerate(
                            ...
                            print_rank_0(
                                f"nidx: {nidx}, prompts_tokens_tensor after[{nidx}]: {tokenizer.detokenize(ns)}"
                            )
                    ...
                    for nidx, ns in enumerate(
                        ...
                        print_rank_0(
                            f"nidx: {nidx}, prompts_tokens_tensor after copy_from_last_to_first_pipeline_stage [{nidx}]: {tokenizer.detokenize(ns)}"
                        )
                    ...
                    done = broadcast_from_last_pipeline_stage(
                        1, torch.uint8, tensor=done
                    )
            ...
            for nidx, ns in enumerate(prompts_tokens_tensor.cpu().numpy().tolist()):
                print_rank_0(
                    f"nidx: {nidx}, detokenized prompts_tokens_tensor after the generate loop [{nidx}]: {tokenizer.detokenize(ns)}"
                )
            prompts_tokens_tensor = prompts_tokens_tensor[:, : (context_length + 1)]
            for nidx, ns in enumerate(prompts_tokens_tensor.cpu().numpy().tolist()):
                print_rank_0(
                    f"nidx: {nidx}, detokenized prompts_tokens_tensor after the generate loop and slicing with ctx length[{nidx}]: {tokenizer.detokenize(ns)}"
                )
            if mpu.is_pipeline_last_stage():
                if return_output_log_probs:
            ...
            for nidx, ns in enumerate(prompts_tokens_tensor.cpu().numpy().tolist()):
                print_rank_0(
                    f"nidx: {nidx}, detokenized prompts_tokens_tensor after the generate loop and befoer final post-process[{nidx}]: {tokenizer.detokenize(ns)}"
                )
            ...
            if mpu.is_pipeline_first_stage():
                ...
                for nidx, ns in enumerate(prompts_tokens_tensor.cpu().numpy().tolist()):
                    print_rank_0(
                        f"nidx: {nidx}, detokenized prompts_tokens_tensor after the generate loop and after final post-process[{nidx}]: {tokenizer.detokenize(ns)}"
                    )
        ...
        if False:
            ...
            tokens_to_generate = 64
            generated_responses = generate_post_training(
                model, prompts, tokens_to_generate, fprint=False
            )
        ...
        if False:
            ...
            try:
                ...
                response, _, _, _ = generate_and_post_process(
                    ...
                    tokens_to_generate=tokens_to_generate_len,
                )
