...
def main():
    ...
    if os.getenv('TORCH_PROFILER_ENABLED') == '1':
        ...
        for i in range(args.train_iters):
            ...
            seq_logps_p = torch.sum(loss_c[:8,:] * loss_mask_p, dim=-1) / torch.sum(loss_mask_p, dim=-1)
            seq_logps_u = torch.sum(loss_c[8:,:] * loss_mask_u, dim=-1) / torch.sum(loss_mask_u, dim=-1)
            rseq_logps_p = torch.sum(rloss_c[:8,:] * loss_mask_p, dim=-1) / torch.sum(loss_mask_p, dim=-1)
            rseq_logps_u = torch.sum(rloss_c[8:,:] * loss_mask_u, dim=-1) / torch.sum(loss_mask_u, dim=-1)
            ...
            # print(f'args.no_pipeline_parallel: {args.no_pipeline_parallel}')
            # if args.deepspeed and args.ds_pipeline_enabled:
            #     print(f'In train step if args.deepspeed and args.ds_pipeline_enabled..')

            # print(f'loss_mask_p: {loss_mask_p}')
