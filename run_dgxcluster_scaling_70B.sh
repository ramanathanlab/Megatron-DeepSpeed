...
srun --mpi=pmix --nodes $SLURM_NNODES --gpus-per-node 8 --ntasks-per-node=8 --container-workdir=${_basedir} --container-mounts="/lustre/fs0/scratch/gdharuman","/home/gdharuman" --container-image=${CONTAINER} python /lustre/fs0/scratch/gdharuman/Megatron-DeepSpeed/dpo_training.py \
   ...
   --use-checkpoint-opt_param-scheduler --lr 5e-6 --seq-length 128 \
   ...
   --num-layers 80 --hidden-size 8192 --train-iters 100 --eval-iters 10 \
   --distributed-backend nccl --num-attention-heads 64 --save-interval 2000 \
   --eval-interval 50000 --max-position-embeddings 128 --micro-batch-size 2 \
   ...
   --num-key-value-heads 8 --data-cache-path ./index-cache \
   --ffn-hidden-size 28672 --tokenizer-model ALCF/tokenizer.model \
   ...
   --zero-stage=2 --deepspeed_config=ds_config-gpt_nooffload_zero2.json \
