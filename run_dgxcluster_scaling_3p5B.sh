...
srun --mpi=pmix --nodes $SLURM_NNODES --gpus-per-node 8 --ntasks-per-node=8 --container-workdir=${_basedir} --container-mounts="/lustre/fs0/scratch/gdharuman","/home/gdharuman" --container-image=${CONTAINER} python /lustre/fs0/scratch/gdharuman/Megatron-DeepSpeed/dpo_training_ref.py \
   ...
   --num-layers 16 --hidden-size 4096 --train-iters 100 --eval-iters 10 \
   --distributed-backend nccl --num-attention-heads 32 --save-interval 2000 \
   --eval-interval 50000 --max-position-embeddings 1024 --micro-batch-size 8 \
